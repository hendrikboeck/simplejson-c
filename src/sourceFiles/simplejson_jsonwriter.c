...
JSONWriter jsonwriter_new(Dict data) {
  ...
  _jsonwriter_serializeDict(self, self->data);
...
void _jsonwriter_serializeObject(JSONWriter self, const Object val) {
  switch (val->type) {
    case NULL_OBJ_TYPE:
      _jsonwriter_serializeNull(self);
      break;

    case INT64_OBJ_TYPE:
      _jsonwriter_serializeInt64(self, object_getInt64(val));
      break;

    case FLOAT64_OBJ_TYPE:
      _jsonwriter_serializeFloat64(self, object_getFloat64(val));
      break;

    case BOOL_OBJ_TYPE:
      _jsonwriter_serializeBool(self, object_getBool(val));
      break;

    case STR_OBJ_TYPE:
      _jsonwriter_serializeStr(self, object_getStr(val));
      break;

    case LIST_OBJ_TYPE:
      _jsonwriter_serializeList(self, object_getList(val));
      break;

    case DICT_OBJ_TYPE:
      _jsonwriter_serializeDict(self, object_getDict(val));
      break;
...
void _jsonwriter_serializeList(JSONWriter self, const List val) {
  ...
  for (size_t i = 0; i < val->len; i++) {
    _jsonwriter_serializeObject(self, list_at(val, i));
...
void _jsonwriter_serializeDict(JSONWriter self, const Dict val) {
  stringbuffer_putChar(self->sbuf, '{');
  for (size_t i = 0; i < val->len; i++) {
    _jsonwriter_serializeStr(self, dict_data(val)[i].key);
    stringbuffer_putChar(self->sbuf, ':');
    _jsonwriter_serializeObject(self, dict_data(val)[i].value);
    if (i < val->len - 1) stringbuffer_putChar(self->sbuf, ',');
  }
  stringbuffer_putChar(self->sbuf, '}');
}
